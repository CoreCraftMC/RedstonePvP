...
import com.ryderbelserion.redstonepvp.managers.StorageManager;
import com.ryderbelserion.redstonepvp.managers.data.Sqlite;
...
public class CommandBeaconAdd extends Command {
    ...
    public void execute(final CommandContext<CommandSourceStack> stack) {
        ...
        // Get the instance.
        final Sqlite instance = StorageManager.getRedstone();

        /*if (!instance.isOpen()) {
            return;
        }*/

        try (PreparedStatement statement = instance.getConnection().prepareStatement("insert into beacon(id) values (?)")) {
